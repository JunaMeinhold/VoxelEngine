namespace HexaEngine.Scenes
{
    ...
    public class Scene
    {
        ...
        public List<HexaElement> Elements { get; } = new();
        ...
        public IDeferredRenderer DeferredRenderer { get; set; }
        ...
        {
            Lights.ForEach(x => x.Initialize());
            Elements.ForEach(x => x.Scene = this);
            Elements.ForEach(x => x.InternalAwake());
            ForwardRenderers.ForEach(x => x.Initialize(DeviceManager));
            Renderers.ForEach(x => x.Initialize(DeviceManager));
        ...
        public void Render()
        {
            ...
            DeferredRenderer.RenderGBuffers();
            Elements.ForEach(x => x.InternalUpdate());
            DeferredRenderer.RenderLights(Lights, Elements);
            DeviceManager.ClearRenderTarget();
            DeviceManager.SetRenderTarget();
            ForwardRenderers.ForEach(x => x.Render(DeviceManager, Camera));
            DeferredRenderer.RenderCompose(Camera);
        }

        private void FixedUpdate(object sender, EventArgs e)
        {
            Elements.ForEach(x => x.FixedUpdate());
        }
        ...
        public void Unload()
        {
            Lights.ForEach(x => x.Uninitialize());
            Elements.ForEach(x => x.InternalDestroy());
            ForwardRenderers.ForEach(x => x.Uninitialize());
            Renderers.ForEach(x => x.Uninitialize());
            Simulation.Dispose();
        ...
        private unsafe struct NarrowPhaseCallbacks : INarrowPhaseCallbacks
        {
            /// <summary>
