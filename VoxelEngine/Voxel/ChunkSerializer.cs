namespace VoxelEngine.Voxel
{
    ...
    public static class ChunkSerializer
    {
        ...
        public static unsafe void Serialize(Stream stream, Chunk2* chunk)
        {
            long begin = stream.Position;
        ...
        public static unsafe void Deserialize(Chunk2* chunk, Stream stream)
        {
            if (!chunk->IsAllocated)
            {
                chunk->Allocate();
            ...
            for (int i = 0; i < recordCount; i++)
            {
                ...
                {
                    int index = record.Index + y;
                    chunk->Voxels[index] = new Block(record.Type);
                }
            }
        }
    }
