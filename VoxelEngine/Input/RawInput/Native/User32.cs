namespace HexaEngine.Input.RawInput.Native
{
    ...
    public static class User32
    {
        ...
        private static extern uint GetRawInputDeviceList([Out] RawInputDeviceListItem[] pRawInputDeviceList, ref uint puiNumDevices, uint cbSize);

        [DllImport("user32", SetLastError = true)]
        private static extern uint GetRawInputDeviceInfo(IntPtr hDevice, RawInputDeviceInfoBehavior uiBehavior, IntPtr pData, out uint pcbSize);

        [DllImport("user32", SetLastError = true, CharSet = CharSet.Unicode)]
        private static extern uint GetRawInputDeviceInfo(IntPtr hDevice, RawInputDeviceInfoBehavior uiBehavior, StringBuilder pData, in uint pcbSize);

        [DllImport("user32", SetLastError = true)]
        private static extern uint GetRawInputDeviceInfo(IntPtr hDevice, RawInputDeviceInfoBehavior uiBehavior, out RawInputDeviceInfo pData, in uint pcbSize);

        [DllImport("user32", SetLastError = true)]
        private static extern uint GetRawInputDeviceInfo(IntPtr hDevice, RawInputDeviceInfoBehavior uiBehavior, [Out] byte[] pData, in uint pcbSize);

        [DllImport("user32", SetLastError = true)]
        private static extern bool RegisterRawInputDevices(RawInputDeviceRegistration[] pRawInputDevices, uint uiNumDevices, uint cbSize);

        [DllImport("user32", SetLastError = true)]
        private static extern uint GetRegisteredRawInputDevices([Out] RawInputDeviceRegistration[] pRawInputDevices, ref uint puiNumDevices, uint cbSize);

        [DllImport("user32", SetLastError = true)]
        private static extern uint GetRawInputData(IntPtr hRawInput, RawInputGetBehavior uiBehavior, IntPtr pData, ref uint pcbSize, uint cbSizeHeader);

        [DllImport("user32", SetLastError = true)]
        private static extern uint GetRawInputData(IntPtr hRawInput, RawInputGetBehavior uiBehavior, out RawInputHeader pData, ref uint pcbSize, uint cbSizeHeader);

        [DllImport("user32", SetLastError = true)]
        private static extern uint GetRawInputBuffer(IntPtr pData, ref uint pcbSize, uint cbSizeHeader);

        [DllImport("user32", SetLastError = true)]
        private static extern IntPtr DefRawInputProc(byte[] paRawInput, int nInput, uint cbSizeHeader);
