...
float4 main(VSOut pin) : SV_TARGET
{
	...
	float3 color = sunTexture.SampleLevel(linearClampSampler, tex_coord, 0).rgb;
	...
	for (int i = 0; i < SAMPLES; i++)
	{
		...
		float3 sam = sunTexture.SampleLevel(linearClampSampler, tex_coord.xy + delta_tex_coord * jitter, 0).rgb;
